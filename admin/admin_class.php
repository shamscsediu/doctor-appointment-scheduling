...
Class Action {
	...
	function save_doctor(){
		...
		if(isset($offer_online)){
			$data .= ", online_checkup = '$offer_online' ";
		}
		if(isset($online_checkup_fee)){
			$data .= ", online_checkup_fee = '$online_checkup_fee' ";
		}
		$data .= ", contact = '$contact' ";
	...
	function set_appointment(){
		...
		if(isset($trx_id) && !empty($trx_id)){
			$data .= ", online = 1, trx_id = '$trx_id', payment_status=1";
		}
