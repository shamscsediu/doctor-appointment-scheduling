					...
					<div class="card">
						...
						<div class="card-body">
							...
							<label class="control-label">Division</label>
								<select name="division" class="custom-select browser-default select2" required>
									<option disabled selected>Select division</option>
									<?php 
									$qry = $conn->query("SELECT * FROM basic_division order by division asc");
										while($row=$qry->fetch_assoc()):
									 ?>
							...
							<div>Offer Online Checkup</div>

							<div class="form-check">
								<input class="form-check-input" type="radio" name="offer_online" id="exampleRadios1"
									value="1" checked>
							...
							<div class="form-check">
								<input class="form-check-input" type="radio" name="offer_online" id="exampleRadios2"
									value="2">
							...
							<div class="form-group">
								<label for="" class="control-label">Online Checkup Fee</label>
								<input type="text" class="form-control" name="online_checkup_fee">
							</div>
							<div class="form-group">
								<img src="" alt="" id="cimg">
							</div>
					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php
								$i = 1;
								$cats = $conn->query("SELECT * FROM doctors_list where id=" . $_SESSION['login_doctor_id'] . " order by id asc");
									...
									<tr>
										...
										<td class="text-center">
											<button class="btn btn-sm btn-primary edit-doctor" type="button"
												...
												data-online_checkup_fee="<?php echo $row['online_checkup_fee'] ?>">Edit</button>
											<p><small><a href="javascript:void(0)" class="view_schedule"
														data-id="<?php echo $row['id'] ?>"
														data-name="<?php echo "Dr. " . $row['name'] . ', ' . $row['name_pref'] ?>"><i
															class='fa fa-calendar'></i> Manage Schedule</a></b></small></p>
										</td>
									</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('.edit-doctor').click(function () {
		...
		cat.find("[name='online_checkup_fee']").val($(this).attr('data-online_checkup_fee'))
		cat.find("[name='contact']").val($(this).attr('data-contact'))
		cat.find("[name='division']").find('option:contains('+$(this).attr('data-division')+')').attr("selected",true);
